...
public class Canvas2DVectorFieldModifier extends JPanel implements Runnable, ActionListener, KeyListener {

	/*
	 * final variables
	 */
	private final ArrayList<VectorField2D> vectorFields2D;
	...
	public Canvas2DVectorFieldModifier(Canvas2D canvas2d) {
		...
		vectorFields2D = canvas2d.get2DVectorFields();
		updateListData();
	...
	private void updateListData() {
		...
		for (int i = 0; i < vectorFields2D.size(); i++) {
			...
			dataIndex = 0;
			v = vectorFields2D.get(i);

			JCheckBox box = new JCheckBox();
			box.setName(Integer.toString(i));
			box.setSelected(v.isVisible());
	...
	private void readAndChangeRange(int functionIndex, String newRange) {
		...
		try {
			range[2] = Double.parseDouble(rangeSplit[2]);
		} catch (NumberFormatException nfe) {
			range[2] = 0;
		}
		try {
			range[3] = Double.parseDouble(rangeSplit[3]);
		} catch (NumberFormatException nfe) {
			range[3] = 0;
		}
		if (range[2] > range[3]) {
		...
		vectorFields2D.get(functionIndex).setRange(range);
	...
	private void readAndChangeColor(int functionIndex, String newColor) {
		...
		vectorFields2D.get(functionIndex).setColor(new Color(r, g, b));
	}
	...
	public void run() {
		...
		while (true) {
			if (vectorFields2D.size() != listData.size()) {
				updateListData();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() instanceof JCheckBox) {
			...
			vectorFields2D.get(Integer.parseInt(j.getName())).setVisible(j.isSelected());
		}

		if (e.getSource() instanceof JButton) {
			JButton b = (JButton) e.getSource();
			vectorFields2D.remove(Integer.parseInt(b.getName()));
